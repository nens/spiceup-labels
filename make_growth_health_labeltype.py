...
if not "growth_info" in locals():
    ...
    growth_info = growth_info.loc[
        ~growth_info.index.duplicated(keep="first")
    ]  # Delere duplicates
    growth_info = growth_info.replace(np.nan, "")
...
periods = pd.read_excel(
    "growth_params.xlsx", sheet_name="periods", index_col="parameter"
)
...
for index, row in periods.iterrows():
    ...
    lowerbounds = [
        month - 0.1
        for month in boundsrow.loc[(boundsrow.index.str.endswith("lower"))]
        if ~np.isnan(month)
    ]
    upperbounds = [
        month
        for month in boundsrow.loc[(boundsrow.index.str.endswith("upper"))]
        if ~np.isnan(month)
    ]
...
for index, row in periods.iterrows():
    for returncolumn in growthreturncolumns:
        ...
        growth_info["tempcol"] = (
            growth_info["strindex"] + "_" + growth_info[returncolumn]
        )
